...
class Produkt {
  ...
  Produkt({required this.nazwa, required this.dataWaznosci, required this.ilosc, required this.jednostka,});
// Dodaj to:
  int dniDoWaznosci() {
  ...
  String toSaveString() => "$nazwa|$dataWaznosci|$ilosc|$jednostka";
  
  // To pozwoli odczytać tekst z dysku i zrobić z niego obiekt Produkt
  factory Produkt.fromSaveString(String data) {
    final parts = data.split('|');
    return Produkt(nazwa: parts[0], dataWaznosci: parts[1], ilosc: int.parse(parts[2]), jednostka: Jednostka.values.byName(parts[3]));
}}